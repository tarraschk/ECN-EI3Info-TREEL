...
//---------------------------------------------------------------------------*
/*
//La routine suivante interroge la liaison série :
// - si aucune donnée n’est prête, la fonction renvoie false et *outCaractere n’est pas modiﬁé ;
// - si une donnée est prête, la fonction renvoie true et *outCaractere contient l’octet reçu.
bool recevoir_caractere_serie1 (uint8 * outCaractere) {
  bool recu = (U1LSR & 0x01) != 0 ;
//---
if (recu) {
    * outCaractere = U1RBR ;
  }
...
//Pour attendre une donnée série, vous pouvez utiliser la routine suivante :
uint8 attente_caractere_serie1 (void) {
  uint8 resultat = '\0' ;
//---
  while (! recevoir_caractere_serie1 (& resultat)) {
  }
//---
  return resultat ;
}

//La routine suivante attend que le transmetteur soit libre, puis envoie la donnée :
void envoyer_caractere_serie1 (const char inCaractere) {
//--- Attendre que l'emetteur soit pret
  while ((U1LSR & 0x20) == 0) ;
